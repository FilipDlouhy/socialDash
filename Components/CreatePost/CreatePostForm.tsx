...
function CreatePostForm() {
  ...
  async function formSumbit (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    let id = uuid()
    if(description && title && img && placeFrom  && userId)
    { 
      ...
      axios.post("/api/createPost",post).then((res)=>{
        ...
        {
          setHeader("Something went wrong pleasy try again")
        }
      })
    }
    else{
  ...
  return (
    <form className='SingInForm flex justify-center items-center flex-col '>
        ...
        <div className='w-2/3 h-32 my-9 flex justify-around items-center flex-col' >
            <label className='text-2xl font-semibold' >Place where the photo was taken</label>
            <select onChange={(e)=>{setPlaceFrom(e.target.value)}} className=' text-white font-medium w-64 h-9 rounded-md bg-blue-300 ' id="country" name="country">
        ...
        <div className='w-2/3 my-9 flex  items-center flex-col' >
            <label  className='text-2xl font-semibold'>Description of Post</label>
            <textarea onChange={(e)=>{setDescription(e.target.value)}} className='rounded-md  text-white font-medium px-3 border-2 w-96 h-72 resize-none  bg-blue-300 '></textarea>
        </div>  


        
        <div className='w-2/3 h-32 my-9 flex justify-around items-center flex-col' >
            <label  className='text-2xl font-semibold'>Choose an Post Img</label>
            <input  onChange={(e)=>{
              if(e.target.files)
              setImg(e.target.files[0])}} className='custom-file-input' type="file" ></input>
        </div>




        <button onClick={(e)=>{formSumbit(e)}} className='hvr-rectangle-in w-56 rounded-lg text-xl font-semibold cursor-pointer h-9 mb-3 bg-blue-300 text-white'>Post</button>
    </form>
  )
}
